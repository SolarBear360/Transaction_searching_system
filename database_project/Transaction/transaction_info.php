...
<?php 
    ...
    function query_result($stm){
        ...
        $item_price = 0;
        $item_remain = 0;
        $stm->bind_result($transaction_ID,$buyer_ID,$item_ID,
                            $transaction_count,$item_ID,$seller_ID,
                            $item_name,$item_price,$item_remain);
        echo "<table border='1'>
        <tr>
            <th>買家ID</th>
            <th>商品ID</th>
...
<body>
    ...
    <form method="POST" action="transaction_info.php">
        所有交易 <input type='hidden' name='all_tran'> <br>
        <button type="submit">查詢</button>
    </form>
...
<?php 
    if(isset($_POST['buyer_ID'])){
        ...
        query_result(($stmt));
    }else if(isset($_POST['all_tran'])){
        $query = ("SELECT * FROM transactions, item WHERE item.item_ID = transactions.item_ID");
        $stmt = $conn->prepare(($query));
        $stmt->execute();
        query_result(($stmt));
    }
